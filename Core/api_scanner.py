...
async def _run_simple_fuzzer(base_url: str, targets: List[Dict[str, Any]], auth_cfg: Dict[str, Any], timeout: int = 15) -> List[ApiFinding]:
    ...
    async with aiohttp.ClientSession() as session:
        print(Fore.GREEN + f"[+] 📢 Running Fuzzer ...", flush=True)
        tasks = []
        for t in targets:
    ...
    print(Fore.GREEN + f"[+] 📢 Fuzzer found {len(findings)} issues.\n", flush=True)
    return findings
...
async def _run_zap_scan(base_url: str, zap_cfg: Dict[str, Any], timeout_sec: int = 300) -> List[ApiFinding]:
    ...
    try:
        print(Fore.CYAN + f"[+] 📢 Running ZAP Scan...", flush=True)
        ...
        print(Fore.CYAN + f"[+] 📢 ZAP Scan found {len(findings)} issues.\n", flush=True)
